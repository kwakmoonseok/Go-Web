...
func TestCreateUser(t *testing.T) {
	...
	res, err := http.Post(ts.URL + "/users", "application/json", 
		strings.NewReader(`{"first_name" : "kinder", "last_name": "kwak", "email" : "example@gmail.com"}`))
	assert.NoError(err)
...
func TestUpdateUser(t *testing.T) {
	...
	req, _ := http.NewRequest("PUT", ts.URL + "/users", 
		strings.NewReader(`{"id":1, "first_name":"updated", "last_name":"updated", "email":"updatedAt@gmail.com"}`))
	res, err := http.DefaultClient.Do(req)
	assert.NoError(err)
	assert.Equal(http.StatusOK, res.StatusCode)
	data, _ := ioutil.ReadAll(res.Body)
